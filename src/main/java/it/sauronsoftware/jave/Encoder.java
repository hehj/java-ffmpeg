...
public class Encoder {
	...
	private MultimediaInfo parseMultimediaInfo(File source,
			...
			EncoderException {
		//Input #0, mov,mp4,m4a,3gp,3g2,mj2, from 'd:\trailer_002winnie.mp4':
...
//				Pattern.CASE_INSENSITIVE);
		Pattern p2 = Pattern.compile(
				"^\\s*Duration: (\\d\\d):(\\d\\d):(\\d\\d)\\.(\\d), start: (\\d+.\\d+), bitrate: (\\d+) kb/s",
				Pattern.CASE_INSENSITIVE);
		
		//Duration: 00:00:55.7, start: 0.000000, bitrate: 1164 kb/s
				...
				if (step == 0) {
					...
					if (m.matches()) {
						...
						long duration = (dec * 100L) + (seconds * 1000L)
								+ (minutes * 60L * 1000L)
								+ (hours * 60L * 60L * 1000L);
						int bitRate = Integer.parseInt(m.group(6));
